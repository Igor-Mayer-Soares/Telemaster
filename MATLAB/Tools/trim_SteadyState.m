...
function SteadyState = trim_SteadyState(Ve,Heq,model)
%% Trimming Points
[V,H] = ndgrid(Ve,Heq);
...
for cont = 1:numel(V)
    ...
    SteadyState(cont).V         = V(cont);
    SteadyState(cont).H         = H(cont);
    SteadyState(cont).ail       = op.Inputs(1, 1).u;
    SteadyState(cont).thr       = op.Inputs(2, 1).u;
    SteadyState(cont).ele       = op.Inputs(3, 1).u;
    SteadyState(cont).rud       = op.Inputs(4, 1).u;
    SteadyState(cont).alpha     = rad2deg(atan2(opreport.States(9, 1).x,opreport.States(7, 1).x));
    SteadyState(cont).trim      = string(opreport.TerminationString) == 'Operating point specifications were successfully met.' && (-10 < SteadyState(cont).alpha) && (SteadyState(cont).alpha < 15);
    SteadyState(cont).op        = op;
    SteadyState(cont).opreport  = opreport;
end
end